"""
주식 검색기 v4.4 (3~5일 스윙 전용)
────────────────────────────────────────
v4.4 → v4.4 변경사항 (Phase 7: 텔레그램 양방향 통신):
...
────────────────────────────────────────
v4.4 → v4.4 변경사항 (Phase 5: 체결강도 필터 + 눌림 깊이 기록):
...
────────────────────────────────────────
v4.4 → v4.4 변경사항 (Phase 4: 신호 강도 점수화):
...
def _send_weekly_report(now: datetime) -> None:
    ...
    print(f"  [주간 리포트] {week_label}: {stats['total']}건 | 승률 {stats['win_rate']*100:.0f}%")
...
def add_positions(stocks: list[dict]) -> None:
    ...
    print(f"  포지션 기록: {len(to_add)}개 추가 (누적 {len(existing)}개 / 최대 {cap}개)")
    ...
    if skipped:
        ...
        print(f"  [SKIP] 한도 초과 {len(skipped)}개 미추가: {names}")
...
def job_second_screen() -> None:
    ...
    for stock in candidates:
        ...
        if ok_vol_dry and ok_support and ok_shape and ok_buy_pressure:
            ...
            print(f"  [통과] {stock['name']} (현재가={cur:,} | 손절폭={sl_pct*100:.1f}% | 체결강도={live_buy_pressure:.0f} | 눌림깊이={pullback_depth:.1f}%)")
        else:
            print(f"  [탈락] {stock['name']} vol={ok_vol_dry} support={ok_support} shape={ok_shape} buy_pressure={ok_buy_pressure}({live_buy_pressure:.0f})")
    ...
    if _pause_signals:
        ...
        print(f"  [PAUSE] 신호 {len(verified)}개 억제됨 (_pause_signals=True)")
    else:
        send_telegram(msg)
        print("  텔레그램 발송 완료")
...
def send_startup_message() -> None:
    ...
    send_telegram(
        f"✅ *스윙 눌림목 검색기 v4.4 시작* (채팅방 {len(TELEGRAM_CHAT_IDS)}개)\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
def _telegram_polling_loop() -> None:
    ...
    if not TELEGRAM_TOKEN:
        print("[polling] 텔레그램 토큰 없음 — 폴링 비활성화")
        return

    allowed_ids = {str(c) for c in TELEGRAM_CHAT_IDS}
    print("[polling] 텔레그램 커맨드 폴링 시작")
    ...
    while True:
        try:
            ...
            for update in res.json().get("result", []):
                ...
                print(f"[polling] 커맨드: {text!r} (chat={chat_id})")
                _handle_command(text)

        except Exception as e:
            print(f"[polling] 예외: {e}")
            time.sleep(5)
...
if __name__ == "__main__":
    ...
    print("\n✅ 스윙 눌림목 검색기 v4.4 시작")
    print("  ⏰ 09:00 → Heartbeat + 만료 이력 기록 (월요일: 주간 리포트)")
    print("  ⏰ 10:00 → 장중 TP/SL 모니터링")
    print("  ⏰ 13:00 → 장중 TP/SL 모니터링")
    print("  ⏰ 14:30 → 1차 스크리닝")
    print("  ⏰ 15:20 → 2차 재검증 + 텔레그램")
    print(f"  🔑 KIS 모드: {'실전투자' if _KIS_MODE == 'real' else '모의투자'}")
    print(f"  🎯 트레일링 스탑: HWM -{int(STRATEGY['trail_pct']*100)}%")
    print(f"  📊 드리프트 감지: {STRATEGY['drift_weeks']}주 연속 승률 {int(STRATEGY['drift_winrate_threshold']*100)}% 미달")
    print("  종료: Ctrl+C\n")
